...
export async function parseMarkdownToVueEmailJSX(markdown: string, customStyles: StylesType = {}, withDataAttr = true): Promise<string> {
  ...
  vueMailTemplate = vueMailTemplate.replace(patterns.codeBlocks, (_, codeContent: string) => {
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h1,
    `<h1${parseCssInJsToInlineCss(finalStyles.h1) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h1)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h1>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h2,
    `<h2${parseCssInJsToInlineCss(finalStyles.h2) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h2)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h2>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h3,
    `<h3${parseCssInJsToInlineCss(finalStyles.h3) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h3)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h3>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h4,
    `<h4${parseCssInJsToInlineCss(finalStyles.h4) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h4)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h4>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h5,
    `<h5${parseCssInJsToInlineCss(finalStyles.h5) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h5)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h5>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h6,
    `<h6${parseCssInJsToInlineCss(finalStyles.h6) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h6)}"` : ''}${withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h6>`,
  )
  ...
  vueMailTemplate = vueMailTemplate.replace(patterns.table, (match: string) => {
    ...
    const body = rows
      ...
      .map((row) => {
        ...
        return `<tr${parseCssInJsToInlineCss(finalStyles.tr) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.tr)}"` : ''}>${cells
          .map(
            (cell, index) =>
              `<td  align="${alignments[index]}"${parseCssInJsToInlineCss(finalStyles.td) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.td)}"` : ''}>${cell}</td>`,
          )
          .join('')}</tr>`
      })
      .join('')

    const table = `<table${parseCssInJsToInlineCss(finalStyles.table) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.table)}"` : ''}><thead${parseCssInJsToInlineCss(finalStyles.thead) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.thead)}"` : ''
      }><tr${parseCssInJsToInlineCss(finalStyles.tr) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.tr)}"` : ''}>${headers
        .map(
          (header, index) =>
            `<th align="${alignments[index]}"${parseCssInJsToInlineCss(finalStyles.th) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.th)}"` : ''}>${header}</th>`,
        )
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.bold,
    `<strong${parseCssInJsToInlineCss(finalStyles.bold) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.bold)}"` : ''}${withDataAttr ? ' data-id="vue-email-text"' : ''
    }>$1</strong>`,
  )
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.italic,
    `<em${parseCssInJsToInlineCss(finalStyles.italic) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.italic)}"` : ''}${withDataAttr ? ' data-id="vue-email-text"' : ''
    }>$2</em>`,
  )
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.link,
    `<a${withDataAttr ? ' data-id="vue-email-link"' : ''}${parseCssInJsToInlineCss(finalStyles.link) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.link)}"` : ''
