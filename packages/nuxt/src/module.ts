...
export interface ModuleOptions {
   ...
   * ```
   * @see https://vuemail.net/getting-started/installation#options
   */
  baseUrl?: string
  /**
...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup (options, nuxt) {
    ...
    nuxt.options.runtimeConfig.public.vueEmail = options || {}
    ...
    addTemplate({
      ...
      getContents: () => ["declare module '#vue-email' {", `  const useCompiler: typeof import('${resolve('./runtime/server/services')}')`, '}'].join('\n'),
    })

    nuxt.hook('prepare:types', (options) => {
      options.references.push({
        path: resolve(nuxt.options.buildDir, 'types/vue-email-compiler.d.ts'),
      })
    })
