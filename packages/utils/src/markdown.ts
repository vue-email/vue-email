...
import type { StylesType } from '@vue-email/types'
...
export async function parseMarkdownToVueEmailJSX(markdown: string, customStyles: StylesType = {}, withDataAttr = true): Promise<string> {
  ...
  function parseMarkdownWithBlockQuotes(markdown: string): string {
    ...
    function parseBlockQuote(match: string) {
      ...
      return `<blockquote${
        parseCssInJsToInlineCss(finalStyles.blockQuote) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.blockQuote)}"` : ''
      }>\n${nestedHTML}\n</blockquote>`
    }
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h1,
    `<h1${parseCssInJsToInlineCss(finalStyles.h1) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h1)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h1>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h2,
    `<h2${parseCssInJsToInlineCss(finalStyles.h2) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h2)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h2>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h3,
    `<h3${parseCssInJsToInlineCss(finalStyles.h3) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h3)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h3>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h4,
    `<h4${parseCssInJsToInlineCss(finalStyles.h4) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h4)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h4>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h5,
    `<h5${parseCssInJsToInlineCss(finalStyles.h5) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h5)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h5>`,
  )
  vueMailTemplate = vueMailTemplate.replace(
    patterns.h6,
    `<h6${parseCssInJsToInlineCss(finalStyles.h6) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.h6)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-heading"' : ''
    }>$1</h6>`,
  )
  ...
  vueMailTemplate = vueMailTemplate.replace(patterns.table, (match: string) => {
    ...
    const table = `<table${parseCssInJsToInlineCss(finalStyles.table) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.table)}"` : ''}><thead${
      parseCssInJsToInlineCss(finalStyles.thead) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.thead)}"` : ''
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.bold,
    `<strong${parseCssInJsToInlineCss(finalStyles.bold) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.bold)}"` : ''}${
  ...
  vueMailTemplate = vueMailTemplate.replace(
    patterns.italic,
    `<em${parseCssInJsToInlineCss(finalStyles.italic) !== '' ? ` style="${parseCssInJsToInlineCss(finalStyles.italic)}"` : ''}${
      withDataAttr ? ' data-id="vue-email-text"' : ''
    }>$2</em>`,
  )
