...
let EMPTY_OBJ = true ? Object.freeze({}) : {}
...
let isSet = (val) => toTypeString(val) === '[object Set]'
let isDate = (val) => toTypeString(val) === '[object Date]'
let isRegExp = (val) => toTypeString(val) === '[object RegExp]'
let isFunction = (val) => typeof val === 'function'
let isString = (val) => typeof val === 'string'
let isSymbol = (val) => typeof val === 'symbol'
let isObject = (val) => val !== null && typeof val === 'object'
let isPromise = (val) => {
...
let isIntegerKey = (key) => isString(key) && key !== 'NaN' && key[0] !== '-' && '' + parseInt(key, 10) === key
let isReservedProp = makeMap(
...
let capitalize = cacheStringFunction((str) => str.charAt(0).toUpperCase() + str.slice(1))
let toHandlerKey = cacheStringFunction((str) => (str ? `on${capitalize(str)}` : ``))
let hasChanged = (value, oldValue) => !Object.is(value, oldValue)
...
let isGloballyWhitelisted = makeMap(GLOBALS_WHITE_LISTED)
...
let isHTMLTag = makeMap(HTML_TAGS)
...
let MAP_KEY_ITERATE_KEY = Symbol(true ? 'Map key iterate' : '')
let ReactiveEffect = class {
...
let isNonTrackableKeys = makeMap(`__proto__,__v_isRef,__isVue`)
...
let shallowReadonlyHandlers = extend({}, readonlyHandlers, {
	get: shallowReadonlyGet,
})
let toShallow = (value) => value
...
let [mutableInstrumentations, readonlyInstrumentations, shallowInstrumentations, shallowReadonlyInstrumentations] = createInstrumentations()
...
let devtoolsComponentAdded = createDevtoolsComponentHook(
	'component:added',
	/* COMPONENT_ADDED */
)
...
let devtoolsPerfStart = createDevtoolsPerformanceHook(
	'perf:start',
	/* PERFORMANCE_START */
)
...
let isSuspense = (type) => type.__isSuspense
var SuspenseImpl = {
	name: 'Suspense',
...
let TransitionHookValidator = [Function, Array]
...
let isAsyncWrapper = (i) => !!i.type.__asyncLoader
...
let onBeforeUpdate = createHook('bu')
var onUpdated = createHook('u')
var onBeforeUnmount = createHook('bum')
var onUnmounted = createHook('um')
let onServerPrefetch = createHook('sp')
let onRenderTriggered = createHook('rtg')
let onRenderTracked = createHook('rtc')
...
let NULL_DYNAMIC_COMPONENT = Symbol.for('v-ndc')
...
let isReservedPrefix = (key) => key === '_' || key === '$'
let hasSetupBinding = (state, key) => state !== EMPTY_OBJ && !state.__isScriptSetup && hasOwn(state, key)
...
let RuntimeCompiledPublicInstanceProxyHandlers = extend({}, PublicInstanceProxyHandlers, {
	get(target, key) {
...
let isSimpleType = makeMap('String,Number,Boolean,Function,Symbol,BigInt')
...
let isInternalKey = (key) => key[0] === '_' || key === '$stable'
let normalizeSlotValue = (value) => (isArray(value) ? value.map(normalizeVNode) : [normalizeVNode(value)])
let normalizeSlot = (key, rawSlot, ctx) => {
	if (rawSlot._n) {
		return rawSlot
	}
...
let isComment = (node) => node.nodeType === 8
...
let isTeleport = (type) => type.__isTeleport
var isTeleportDisabled = (props) => props && (props.disabled || props.disabled === '')
let isTargetSVG = (target) => typeof SVGElement !== 'undefined' && target instanceof SVGElement
let resolveTarget = (props, select) => {
...
let createVNodeWithArgsTransform = (...args) => {
	return _createVNode(...(vnodeArgsTransformer ? vnodeArgsTransformer(args, currentRenderingInstance) : args))
}
var InternalObjectKey = `__vInternal`
var normalizeKey = ({ key }) => (key != null ? key : null)
let normalizeRef = ({ ref: ref2, ref_key, ref_for }) => {
...
let emptyAppContext = createAppContext()
var uid = 0
...
let ssrContextKey = Symbol.for('v-scx')
var useSSRContext = () => {
	{
...
let templateContainer = doc && doc.createElement('template')
let nodeOps = {
...
let optionsModifierRE = /(?:Once|Passive|Capture)$/
...
let defineSSRCustomElement = (options) => {
...
let Transition = (props, { slots }) => h(BaseTransition, resolveTransitionProps(props), slots)
Transition.displayName = 'Transition'
let DOMTransitionPropsValidators = {
	name: String,
	type: String,
	css: {
		type: Boolean,
...
let positionMap = /* @__PURE__ */ new WeakMap()
...
let systemModifiers = ['ctrl', 'shift', 'alt', 'meta']
let modifierGuards = {
