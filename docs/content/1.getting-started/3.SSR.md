...
Tho this feature is experimental, it is still very powerful, and can be used to render and send emails from the server side, while using the same templates you use on the client side, without the auto import features, as its a bit tricky to implement, and will take some time to get right.
