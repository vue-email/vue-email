...
<${name}`
  for (const [key, value] of Object.entries(componentProps)) {
    ...
    code += ` ${(prop?.type === 'boolean' && value !== true) || typeof value === 'object' ? ':' : ''}${key === 'modelValue' ? 'value' : useKebabCase(key)}${
      prop?.type === 'boolean' && !!value && key !== 'modelValue' ? '' : `="${typeof value === 'object' ? renderObject(value) : value}"`
    }`
  }
...
const { data: ast } = await useAsyncData(
  ...
  () =>
    transformContent('content:_markdown.md', code.value, {
      highlight: {
        theme: {
...
<template>
  <div>
    ...
    <div
      ...
      :class="[{ 'p-4': padding }, propsToSelect.length ? 'border-t-0' : 'rounded-t-md', backgroundClass, overflowClass]"
    >
