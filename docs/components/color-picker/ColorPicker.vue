...
const primaryColors = computed(() =>
  useWithout(appConfig.ui.colors, 'primary').map((color) => ({ value: color, text: color, hex: colors[color][colorMode.value === 'dark' ? 400 : 500] })),
)
...
const grayColors = computed(() =>
  ['slate', 'cool', 'zinc', 'neutral', 'stone'].map((color) => ({ value: color, text: color, hex: colors[color][colorMode.value === 'dark' ? 400 : 500] })),
)
const gray = computed({
  get() {
