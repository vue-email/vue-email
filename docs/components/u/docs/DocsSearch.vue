...
import type { Button, Command } from '@nuxthq/ui/dist/runtime/types'

const props = defineProps<{ files?: ParsedContent[]; links?: Link[] }>()

const router = useRouter()
const { usingInput } = useShortcuts()
const { isDocsSearchOpen } = useUIKit()
  ...
  () =>
    props.links?.map((link) => ({
      ...
      commands: props.files
        .filter((file) => file._path?.startsWith(link.to))
        .map((file) => ({
          id: file._id,
          label: file.navigation?.title || file.title,
          to: file._path,
          suffix: file.description,
          icon: file.navigation?.icon || file.icon,
        })),
    })),
)
...
const canToggleModal = computed(() => isDocsSearchOpen.value || !usingInput.value)
...
defineShortcuts({
  meta_k: {
    ...
    handler: () => {
      isDocsSearchOpen.value = !isDocsSearchOpen.value
    },
  },
  escape: {
    usingInput: true,
    whenever: [isDocsSearchOpen],
    handler: () => {
      isDocsSearchOpen.value = false
    },
  },
...
<template>
  ...
  >
    <UCommandPalette
      ...
      :groups="groups"
      :close-button="{ icon: 'i-heroicons-x-mark-20-solid', color: 'gray', variant: 'ghost', size: 'sm' } as unknown as Button"
